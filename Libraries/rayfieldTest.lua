		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			if Dropdown.List.Visible then
				...
				for _, DropdownOpt in ipairs(Dropdown.List:GetChildren()) do
					if DropdownOpt.ClassName == "Frame" and DropdownOpt.Name ~= "Placeholder" then
						TweenService:Create(DropdownOpt, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {
							BackgroundTransparency = 1
						}):Play()
		
						if DropdownOpt:FindFirstChild("UIStroke") then
							TweenService:Create(DropdownOpt.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {
								Transparency = 1
							}):Play()
						end
		
						if DropdownOpt:FindFirstChild("Title") then
							TweenService:Create(DropdownOpt.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {
								TextTransparency = 1
							}):Play()
						end
					end
			...
			local function FilterDropdown(text)
				...
				for _, child in ipairs(Dropdown.List:GetChildren()) do
					if child.ClassName == "Frame" and child.Name ~= "Placeholder" and child.Name ~= "DropdownSearch" then
						if text == "" or string.find(string.lower(child.Name), text) then
							child.Visible = true
						else
							child.Visible = false
						end
			...
			DropdownSearch.Input:GetPropertyChangedSignal("Text"):Connect(function()
				FilterDropdown(DropdownSearch.Input.Text)
			end)
			-- end FILTER (ADDED)
