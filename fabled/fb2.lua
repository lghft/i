...
--[[
    Roblox Autofarm Script with Continuous Tween and Temporary Platform
    - Continuously updates tween target for smooth following above enemy (no stop/jitter)
    - Spawns a temporary invisible anchored platform under the player while tweening
...
local function spawnPlatformUnderPlayer()
    if tempPlatform and tempPlatform.Parent then return end
    tempPlatform = Instance.new("Part")
    tempPlatform.Anchored = true
    tempPlatform.CanCollide = true
    tempPlatform.Transparency = 1 -- invisible
    tempPlatform.Size = Vector3.new(8, 1, 8)
    tempPlatform.Name = "AutoFarmTempPlatform"
    tempPlatform.Parent = workspace
end

local function updatePlatformPosition(targetPos)
    if tempPlatform and tempPlatform.Parent then
        tempPlatform.Position = Vector3.new(targetPos.X, targetPos.Y - 3, targetPos.Z)
    end
end
...
-- Continuous movement loop (no discrete tweens)
        ...
        if autofarmActive and remainingEnemies.Value > 0 then
            ...
            if enemy then
                while enemy.Parent == enemiesFolder and enemy.Humanoid.Health > 0 and autofarmActive do
                    local targetPos = getSafeAboveEnemyPos(enemy, heightAboveEnemy)
                    if targetPos then
                        spawnPlatformUnderPlayer()
                        updatePlatformPosition(targetPos)
                        -- Move smoothly toward targetPos
                        local currentPos = humanoidRootPart.Position
                        local newPos = currentPos:Lerp(targetPos, math.clamp(0.15, 0, 1))
