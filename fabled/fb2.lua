...
--[[
    Roblox Autofarm Script with 360-Degree Rotation Around Enemy and X Distance Setting
    - Rotates the player around the enemy in a 360-degree loop at a configurable X distance
    - Adds a textbox to the GUI for setting the X distance
    - Removes dodge and platform systems
...
local xDistance = config.xDistance
...
local xDistLabel = Instance.new("TextLabel")
xDistLabel.Size = UDim2.new(0, 120, 0, 25)
xDistLabel.Position = UDim2.new(0, 10, 0, 110)
xDistLabel.BackgroundTransparency = 1
xDistLabel.Text = "X Distance:"
xDistLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
xDistLabel.Font = Enum.Font.SourceSans
xDistLabel.TextSize = 16
xDistLabel.TextXAlignment = Enum.TextXAlignment.Left
xDistLabel.Parent = frame

local xDistBox = Instance.new("TextBox")
xDistBox.Size = UDim2.new(0, 50, 0, 25)
xDistBox.Position = UDim2.new(0, 140, 0, 110)
xDistBox.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
xDistBox.Text = tostring(xDistance)
xDistBox.TextColor3 = Color3.new(1,1,1)
xDistBox.Font = Enum.Font.SourceSans
xDistBox.TextSize = 16
xDistBox.ClearTextOnFocus = false
xDistBox.Parent = frame
...
loadButton.Position = UDim2.new(0, 90, 0, 145)
...
resetButton.Position = UDim2.new(0, 170, 0, 145)
...
xDistBox.FocusLost:Connect(function(enterPressed)
    local val = tonumber(xDistBox.Text)
    if val and val >= 0 then
        xDistance = val
        config.xDistance = val
        xDistBox.Text = tostring(val)
    else
        xDistBox.Text = tostring(xDistance)
    end
end)
...
local ROTATION_SPEED = math.rad(90) -- radians per second (90 degrees/sec)
        ...
        if autofarmActive and remainingEnemies.Value > 0 then
            ...
            if enemy then
                while enemy.Parent == enemiesFolder and enemy.Humanoid.Health > 0 and autofarmActive do
                    ...
                    rotationAngle = (rotationAngle + ROTATION_SPEED * dt) % (2 * math.pi)
                    local enemyPos = enemy.HumanoidRootPart.Position
                    local offset = Vector3.new(
                        math.cos(rotationAngle) * xDistance,
                        heightAboveEnemy,
                        math.sin(rotationAngle) * xDistance
                    )
                    local targetPos = enemyPos + offset
                    humanoidRootPart.CFrame = CFrame.new(targetPos, enemyPos + Vector3.new(0, heightAboveEnemy, 0))
                end
            else
                wait(0.5)
            end
