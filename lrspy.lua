...
-- sSpy Remote Logger Addon for SimpleSpy
-- Place this after SimpleSpy is loaded

-- Ensure sSpy folder exists
local folderName = "sSpy"
if not isfolder(folderName) then
    makefolder(folderName)
end

-- Utility: Sanitize file names (remove invalid characters)
local function sanitize(str)
    return tostring(str):gsub("[^%w_%-%.]", "_")
end

-- Utility: Generate a unique file name for each remote log
local function getFileName(remote, method, id, args)
    local remoteName = sanitize(remote.Name or "Remote")
    local remoteClass = sanitize(remote.ClassName or "Unknown")
    local remoteId = sanitize(id or "unknown")
    -- Hash args for uniqueness (since args can be long, use JSON)
    local argsStr = ""
    pcall(function()
        argsStr = game:GetService("HttpService"):JSONEncode(args)
    end)
    local argsHash = tostring(argsStr):gsub("[^%w]", ""):sub(1, 16)
    local timestamp = os.time()
    return string.format("%s/%s_%s_%s_%s_%d.txt", folderName, remoteName, remoteClass, remoteId, argsHash, timestamp)
end

-- Save the original newRemote
local oldNewRemote = newRemote

function newRemote(type, data)
    -- Call the original function
    local result = oldNewRemote(type, data)

    -- Generate the script
    local scriptStr = genScript(data.remote, data.args or {})
    -- Compose file name
    local fileName = getFileName(data.remote, data.method, data.id, data.args or {})
    -- Write to file
    writefile(fileName, scriptStr)

    return result
end

print("[sSpy] Remote logger active. All detected remotes will be logged to the 'sSpy' folder.")
